# R_Toolkit (β version)
![beta](https://img.shields.io/badge/R_Toolkit-β版_開発中-orange)

ImageJ macro toolset for calculating the roundness parameter R from particle images.
> **⚠️ This software is currently under development and released as a β version.  
> 機能は開発中であり、β版として提供されています。**

## About

R_Toolkit provides two ImageJ macros for semi-automatic measurement of particle roundness:

- **R_Lab**: Full-featured macro with scale calibration, particle detection, and version management
- **R_Calc**: Lightweight batch processor for high-throughput R calculation

## Documentation

- [R_Lab Documentation](R_Lab/README.md) (English)
- [R_Lab Documentation](R_Lab/README_ja.md) (in Japanese)
- [R_Calc Documentation](R_Calc/README.md) (English)
- [R_Calc Documentation](R_Calc/README_ja.md) (in Japanese)

## Citation

If you use R_Toolkit, please cite:

> Takashimizu, Y. & Iiyoshi, M., 2016,  
> New parameter of roundness *R*: circularity corrected by aspect ratio.  
> *Progress in Earth and Planetary Science*, **3**, 2.  
> DOI: [10.1186/s40645-015-0078-x](https://doi.org/10.1186/s40645-015-0078-x)

## Software Citation

[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.20316300.svg)](https://doi.org/10.5281/zenodo.20316300)

## License

MIT License - See LICENSE file for details

## Contact

For questions, bug reports, or feature requests, please open an [Issue](https://github.com/Yasu-Takashimi/R_Toolkit/issues) on GitHub.

---

*Developed with AI coding assistance (Claude, Anthropic)*
